...
import type {
  PostActiveMobilityAndCarCatchmentArea,
  PostPTCatchmentArea,
...
export const computeCarCatchmentArea = async (
  ...
  const response = await fetchWithAuth(
    `${CAR_CATCHMENT_AREA_API_BASE_URL}?project_id=${projectId}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
...
export const computePTCatchmentArea = async (
  body: PostPTCatchmentArea,
  projectId: string,
) => {
  const response = await fetchWithAuth(
    `${PT_CATCHMENT_AREA_API_BASE_URL}?project_id=${projectId}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
